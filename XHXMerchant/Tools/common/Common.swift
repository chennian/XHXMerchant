...
func countWidth(text : String,size: CGSize = CGSize(width: CGFloat.greatestFiniteMagnitude, height: CGFloat.greatestFiniteMagnitude),font : UIFont) -> CGSize{
    ...
    return (text as NSString).boundingRect(with: size, options: NSStringDrawingOptions.usesLineFragmentOrigin, attributes: dic as [NSAttributedStringKey : Any], context: nil).size
}
