//
//  XPayPasswordStepTwoController.swift
//  XHXMerchant
//
...
class XPayPasswordStepTwoController: SNBaseViewController {
    ...
    @objc func confirm(){
        ...
        SNRequestBool(requestType: API.alterPwd(mobile: mobile, code: code, password: self.confirmField.text!)).subscribe(onNext: {[unowned self] (result) in
            switch result{
            case .bool(_):
