//
//  XPayPasswordStepOneController.swift
//  XHXMerchant
//
...
class XPayPasswordStepOneController: SNBaseViewController {
    ...
    @objc func verify(){
        ...
        SNRequestBool(requestType: API.verifyCode(mobile: self.phoneField.text!,code:self.codeField.text!,vtype:"8")).subscribe(onNext: {[unowned self] (result) in
            switch result{
            case .bool(_):
                
                let vc = XPayPasswordStepTwoController()
    ...
    func sendSMS(){
        ...
        SNRequestBool(requestType: API.sendSMS(mobile: self.phoneField.text!,vtype:"8")).subscribe(onNext: {[unowned self] (result) in
            switch result{
            case .bool(_):
