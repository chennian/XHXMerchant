...
class XServiceTopCell: SNBaseTableViewCell {
    ...
    var activieLable = UILabel().then{
        $0.text = "激活码"
        $0.textColor = Color(0x313131)
        $0.font = Font(30)
    }
    var activieLableField = UITextField().then{
        $0.borderStyle = .none
        $0.font = Font(30)
        $0.textColor = Color(0x313131)
    ...
    override func setupView() {
        ...
        contentView.addSubview(activieLable)
        contentView.addSubview(activieLableField)
        ...
        activieLable.snp.makeConstraints { (make) in
            make.left.equalToSuperview().snOffset(30)
            make.top.equalTo(line3.snp.bottom).snOffset(30)
            make.width.snEqualTo(125)
        }
        
        activieLableField.snp.makeConstraints { (make) in
            make.left.equalTo(activieLable.snp.right).snOffset(30)
            make.centerY.equalTo(activieLable.snp.centerY)
            make.width.snEqualTo(300)
        }
