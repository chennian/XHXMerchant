//
//  XServiceRevenueController.swift
//  XHXMerchant
//
...
class XServiceRevenueController: SNBaseViewController {
    
    var model:[twoRevenueModel] = []
    ...
    func getData() {
        ...
        SNRequest(requestType: API.serviceRevenue(start_date: self.start_date, end_date: self.end_date), modelType: [RevenueTwoModel.self]).subscribe(onNext: {[unowned self] (result) in
            switch result{
...
extension XServiceRevenueController:UITableViewDataSource,UITableViewDelegate{
...
extension XServiceRevenueController:UITextFieldDelegate{
