//
//  XCenterRevenueController.swift
//  XHXMerchant
//
...
class XCenterRevenueController: SNBaseViewController {
    ...
    func getData() {
        ...
        SNRequest(requestType: API.operatorRevenue(start_date: self.start_date, end_date: self.end_date), modelType: [RevenueTwoModel.self]).subscribe(onNext: {[unowned self] (result) in
            switch result{
...
extension XCenterRevenueController:UITableViewDataSource,UITableViewDelegate{
...
extension XCenterRevenueController:UITextFieldDelegate{
