...
class XFlowRevenueController:SNBaseViewController {
    ...
    func getData() {
        ...
        SNRequest(requestType: API.flowMeterRevenue(start_date: self.start_date, end_date: self.end_date), modelType: [RevenueOneModel.self]).subscribe(onNext: {[unowned self] (result) in
            switch result{
...
extension XFlowRevenueController:UITableViewDataSource,UITableViewDelegate{
...
extension XFlowRevenueController:UITextFieldDelegate{
